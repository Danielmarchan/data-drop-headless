...
export default function UploadSelect({
  ...
  const label = selected?.title ?? (isLoading ? 'Loading uploads…' : 'Select an upload');
    ...
    >
      ...
      {open ? (
        <>
          ...
          <ul
            role="listbox"
            className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-lg bg-surface-lowest py-1 shadow-card"
          >
            {uploads.map((u) => {
              ...
              return (
                <li key={u.id}>
                  <button
                    ...
                    className={`flex w-full items-center px-4 py-2.5 text-left font-inter text-sm transition-colors hover:bg-surface-low ${
                      isSelected
